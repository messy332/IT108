...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex space-x-3">
            <a href="{{ route('progress-logs.edit', $progressLog) }}" 
            class=" text-clack px-4 py-2 font-medium hover:underline">
                Edit Log
            </a>
            <a href="{{ route('crops.show', $progressLog->crop) }}" 
                class=" text-clack px-4 py-2 font-medium hover:underline">
                View Crop
            </a>
            ...
            <form id="delete-form" action="{{ route('progress-logs.destroy', $progressLog) }}" method="POST" class="hidden">
                @csrf
                @method('DELETE')
            </form>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="text-sm font-medium text-gray-500">Farmer</label>
                    <p class="text-gray-900">{{ $progressLog->crop->farmer->name }}</p>
                    <p class="text-sm text-gray-500">{{ $progressLog->crop->farmer->email }}</p>
                </div>
