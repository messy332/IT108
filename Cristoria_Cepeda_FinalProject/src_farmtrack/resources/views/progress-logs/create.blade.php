...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('progress-logs.store') }}" method="POST" id="progressLogForm" novalidate>
            @csrf
            <div class="mb-6">
                <h1 class="text-3xl font-bold text-gray-900 flex justify-center">PROGRESS LOG</h1>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-4 gap-2">
                <!-- Crop Selection -->
                <div class="md:col-span-4">
                    <label for="crop_search" class="block text-sm font-medium text-gray-700 mb-2">Select Crop <span class="text-red-500">*</span></label>
                    <input type="text" id="crop_search" placeholder="Search crop by name, variety, or farmer..." 
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 mb-2">
                    <select name="crop_id" id="crop_id" 
                            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                            data-existing-logs="{{ json_encode($existingLogs) }}">
                        ...
                        @foreach($crops as $crop)
                            <option 
                                ...
                                data-crop-name="{{ strtolower($crop->crop_name) }}"
                                data-variety="{{ strtolower($crop->variety) }}"
                                data-farmer="{{ strtolower($crop->farmer->name) }}"
                                {{ old('crop_id', request('crop_id')) == $crop->id ? 'selected' : '' }}
                            >
                ...
                <div>
                    ...
                    <input type="date" name="log_date" id="log_date" value="{{ old('log_date', date('Y-m-d')) }}" 
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500" required>
                    @error('log_date')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="weather_condition" id="weather_condition" 
                        ...
                        @foreach(\App\Models\ProgressLog::getWeatherConditions() as $key => $label)
                            <option value="{{ $key }}" {{ old('weather_condition') === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-4">
                    ...
                    <textarea name="description" id="description" rows="3" 
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="growth_stage" id="growth_stage" value="{{ old('growth_stage') }}" 
                ...
                <div class="md:col-span-3">
                    ...
                    <textarea name="observations" id="observations" rows="1" 
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500">{{ old('observations') }}</textarea>
                    @error('observations')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
...
@endsection
@push('scripts')
@vite('resources/js/progress-logs-create.js')
<script>
(function() {
    function validateProgressLogForm(e) {
        ...
        var activitySel = document.getElementById('activity_type');
        if (!activitySel || !activitySel.value) {
            showError('activityError', 'This input field is required');
            addBorderError(activitySel);
            isValid = false;
        } else {
            hideError('activityError');
            removeBorderError(activitySel);
        }
