...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    <h1 class="text-3xl font-bold text-gray-900 mb-4">My Progress Logs</h1>
    <div class="mb-6 space-y-3">
        <form id="filterForm" method="GET" action="{{ route('farmer.progress-logs.index') }}">
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                <select name="activity_type" class="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 sm:w-44">
                    <option value="">All Activities</option>
                    <option value="planting" {{ request('activity_type') === 'planting' ? 'selected' : '' }}>Planting</option>
                    <option value="watering" {{ request('activity_type') === 'watering' ? 'selected' : '' }}>Watering</option>
                    <option value="fertilizing" {{ request('activity_type') === 'fertilizing' ? 'selected' : '' }}>Fertilizing</option>
                    <option value="weeding" {{ request('activity_type') === 'weeding' ? 'selected' : '' }}>Weeding</option>
                    <option value="pest_control" {{ request('activity_type') === 'pest_control' ? 'selected' : '' }}>Pest Control</option>
                    <option value="harvesting" {{ request('activity_type') === 'harvesting' ? 'selected' : '' }}>Harvesting</option>
                </select>

                <select name="growth_stage" class="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 sm:w-40">
                    <option value="">All Stages</option>
                    @for($i = 1; $i <= 10; $i++)
                        <option value="{{ $i }}" {{ request('growth_stage') == $i ? 'selected' : '' }}>Stage {{ $i }}</option>
                    @endfor
                </select>
            </div>
            ...
            <div class="flex flex-wrap gap-2 justify-end mt-3">
                ...
                @if(request()->hasAny(['q', 'activity_type', 'growth_stage']))
                    <a href="{{ route('farmer.progress-logs.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-2 rounded-md font-medium text-center whitespace-nowrap">
                        Clear
                    </a>
                @endif
                <a href="{{ route('farmer.progress-logs.create') }}" class="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-md font-medium text-center whitespace-nowrap">
                    Add New Log
                </a>
            </div>
        </form>
                    ...
                    @forelse($progressLogs as $log)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('farmer.progress-logs.show', $log) }}" class="text-blue-600 hover:text-blue-900 mr-3 inline-block" title="View">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                    </svg>
                                </a>
                                <a href="{{ route('farmer.progress-logs.show', $log) }}" class="text-green-600 hover:text-green-900 mr-3 inline-block" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                ...
                                <button type="button" class="text-red-600 hover:text-red-900 inline-block" title="Delete"
                                        onclick="openDeleteModal('{{ $log->id }}', '{{ ucfirst(str_replace('_', ' ', $log->activity_type)) }} log ({{ $log->log_date->format('M j, Y') }})')">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                    </svg>
                                </button>
                                <form id="delete-form-{{ $log->id }}" action="{{ route('farmer.progress-logs.destroy', $log) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-4 text-center text-gray-500">
                                No progress logs found. <a href="{{ route('farmer.progress-logs.create') }}" class="text-purple-600 hover:text-purple-900">Add your first log</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
