...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    <h1 class="text-3xl font-bold text-gray-900 mb-4">Crops Management</h1>
    <div class="mb-6 space-y-3">
        <form id="crops-filter-form" method="GET" action="{{ route('crops.index') }}">
            <div class="flex flex-col sm:flex-row gap-3">
                <div class="relative flex-1">
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Search crops..." class="w-full pl-9 pr-3 py-2 border border-gray-300 hover:border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <select name="variety" class="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:w-48">
                    <option value="">All Varieties</option>
                    @foreach($varieties as $v)
                        <option value="{{ $v }}" {{ request('variety') == $v ? 'selected' : '' }}>{{ $v }}</option>
                    @endforeach
                </select>

                <select name="status" class="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:w-40">
        ...
        <div class="flex flex-wrap gap-2">
            <button type="submit" form="crops-filter-form" class="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-md font-medium whitespace-nowrap">
                Apply Filters
            </button>
            @if(request()->hasAny(['q', 'variety', 'status']))
                <a href="{{ route('crops.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-2 rounded-md font-medium text-center whitespace-nowrap">
                    Clear
                </a>
            @endif
            <a href="{{ route('crops.create') }}" class="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-md font-medium whitespace-nowrap">
                Add New Crop
            </a>
        </div>
    </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($crops as $crop)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $crop->farmer->name }}</div>
                                <div class="text-sm text-gray-500">{{ $crop->farmer->email }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $crop->progress_percentage }}%</div>
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-blue-600 h-2 rounded-full" style="width: {{ $crop->progress_percentage }}%"></div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $crop->progress_logs_count }} logs
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('crops.show', $crop) }}" class="text-blue-600 hover:text-blue-900 mr-3 inline-block" title="View">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                    </svg>
                                </a>
                                <a href="{{ route('crops.show', $crop) }}" class="text-green-600 hover:text-green-900 mr-3 inline-block" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                ...
                                <form id="delete-form-{{ $crop->id }}" action="{{ route('crops.destroy', $crop) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-4 text-center text-gray-500">
                                No crops found. <a href="{{ route('crops.create') }}" class="text-blue-600 hover:text-blue-900">Add the first crop</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
