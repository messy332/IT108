        ...
        <form action="{{ route('crops.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-2">
                ...
                <div class="md:col-span-4">
                    <label for="farmer_search" class="block text-sm font-medium text-gray-700 mb-2">Select Farmer <span class="text-red-500">*</span></label>
                    <input type="text" id="farmer_search" placeholder="Search farmer by name..." 
                    ...
                    <select name="farmer_id" id="farmer_id" 
                            ...
                            @if($errors->has('farmer_id')) aria-invalid="true" @endif>
                        <option value="">Choose a farmer</option>
                        @foreach($farmers as $farmer)
                            <option value="{{ $farmer->id }}" data-name="{{ strtolower($farmer->name) }}" data-email="{{ strtolower($farmer->email) }}" {{ old('farmer_id', request('farmer_id')) == $farmer->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date" name="expected_harvest_date" id="expected_harvest_date" value="{{ old('expected_harvest_date') }}" 
                           class="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 cursor-not-allowed" 
                           readonly
                           @if($errors->has('expected_harvest_date')) aria-invalid="true" @endif>
                    @error('expected_harvest_date')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    {{-- <p class="text-gray-500 text-xs mt-1">Auto-calculated based on planting date and variety</p> --}}
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="expected_yield" id="expected_yield" value="{{ old('expected_yield') }}" 
                           class="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 cursor-not-allowed" 
                           readonly
                           @if($errors->has('expected_yield')) aria-invalid="true" @endif>
                    @error('expected_yield')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    {{-- <p class="text-gray-500 text-xs mt-1">Auto-calculated based on area planted and variety</p> --}}
                </div>
            </div>
